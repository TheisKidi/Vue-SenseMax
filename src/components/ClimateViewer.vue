<template>
  <div class="climate-grid-container">
    <div class="climate-grid-left">
      <a v-for="artwork in artworks" :key="artwork.id" class="artwork-container" @click="showArtwork(artwork)"
        href="#app">
    ...
    <div class="climate-grid-right">
      <div v-if="selectedArtwork" class="selected-artwork-container">
        <div class="selected-artwork-background"
          :style="{ backgroundImage: 'url(' + selectedArtwork.artworkPicture + ')' }"></div>
        <div class="selected-artwork-info-container">
          ...
          <div class="selected-artwork-text-container">
            ...
            <p class="artwork-text-content">Grænseværdi, temperatur: [{{ selectedArtwork.minTemp }} - {{
              selectedArtwork.maxTemp }}] °C</p>
            <p class="artwork-text-content">Grænseværdi, fugtighed: [{{ selectedArtwork.minHumidity }} - {{
              selectedArtwork.maxHumidity }}] %</p>
          </div>
        </div>
      </div>
    </div>
...
.artwork-container {
  ...
  display: flex;
  cursor: pointer;
  /* Add cursor style for clickable items */
